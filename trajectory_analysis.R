...
dat = read.csv("touch_screen_data.csv")
...
sum_outcomes = function(dv, practice = T) {
  if (practice) {
    dat$temp = dplyr::pull(dat, dv)
    dat %>%
      dplyr::group_by( id, practice, condition) %>%
    ...
    dat$temp = dplyr::pull(dat, dv)
    dat %>%
      dplyr::group_by(blocking,  id, condition) %>%
      dplyr::summarize(mean_id = mean(temp, na.rm = T)) %>%
...
plot_outcomes(21:33, "good_response_time", "response time")
...
plot_outcomes(21:33, "good_movement_time", "movement time")
...
dat$touch_screen_error = ifelse(
  (dat$id == 1 & (dat$ez_trial %in% c(1, 3, 20, 24)))
  | (dat$id == 2 & (dat$ez_trial %in% c())) 
  | (dat$id == 3 & (dat$ez_trial %in% c()))
  | (dat$id == 4 & (dat$ez_trial %in% c(12, 29, 39, 42, 44, 51, 53)))
  | (dat$id == 5 & (dat$ez_trial %in% c()))
  | (dat$id == 6 & (dat$ez_trial %in% c(57)))
  | (dat$id == 7 & (dat$ez_trial %in% c(11, 18, 52, 60))) 
  | (dat$id == 8 & (dat$ez_trial %in% c(11, 12, 14, 17, 22, 26)))
  | (dat$id == 9 & (dat$ez_trial %in% c(17, 20, 22:27, 30, 50)))
  | (dat$id == 10 & (dat$ez_trial %in% c())) 
  | (dat$id == 11 & (dat$ez_trial %in% c(25, 31)))
  | (dat$id == 12 & (dat$ez_trial %in% c(19, 58)))
  | (dat$id == 13 & (dat$ez_trial %in% c()))
  | (dat$id == 14 & (dat$ez_trial %in% c(23, 27, 45, 47)))
  | (dat$id == 15 & (dat$ez_trial %in% c(11, 15, 38, 48)))
  | (dat$id == 16 & (dat$ez_trial %in% c(40, 45)))
  | (dat$id == 17 & (dat$ez_trial %in% c(17, 22, 24, 33, 34, 36, 39,51)))  
  | (dat$id == 18 & (dat$ez_trial %in% c()))
  | (dat$id == 19 & (dat$ez_trial %in% c()))
  | (dat$id == 20 & (dat$ez_trial %in% c(18))) 
  | (dat$id == 21 & (dat$ez_trial %in% c(15, 54, 55))) 
  | (dat$id == 22 & (dat$ez_trial %in% c(16, 40, 45)))
  | (dat$id == 23 & (dat$ez_trial %in% c()))
  | (dat$id == 24 & (dat$ez_trial %in% c(12, 15, 16, 31, 41, 43, 50, 51, 52, 53, 55, 56)))
  | (dat$id == 25 & (dat$ez_trial %in% c(7, 22))) 
  | (dat$id == 26 & (dat$ez_trial %in% c(21)))
  | (dat$id == 27 & (dat$ez_trial %in% c(31))) 
  | (dat$id == 28 & (dat$ez_trial %in% c()))
  | (dat$id == 29 & (dat$ez_trial %in% c(10, 11, 19, 23, 36, 41, 48)))
  | (dat$id == 30 & (dat$ez_trial %in% c(18, 21, 23, 28)))
  | (dat$id == 31 & (dat$ez_trial %in% c(5, 16, 32, 39, 46, 52, 54)))
  | (dat$id == 32 & (dat$ez_trial %in% c(7, 18, 30, 40, 46)))
  | (dat$id == 33 & (dat$ez_trial %in% c(22, 24, 32)))
  , TRUE
  , FALSE
  )

# count the number of touch screen errors in experimental trials
dat %>%
...
dat %>%
  dplyr::group_by( id, practice, condition, touch_screen_error) %>%
  dplyr::summarize(mean_id_too_soon = mean(too_soon)) %>%
...
dat$optotrak_error = ifelse(
  (dat$id == 1 & (dat$ez_trial %in% c())) 
  | (dat$id == 2 & (dat$ez_trial %in% c())) 
  | (dat$id == 3 & (dat$ez_trial %in% c())) 
  | (dat$id == 4 & (dat$ez_trial %in% c(4))) 
  | (dat$id == 5 & (dat$ez_trial %in% c(30))) 
  | (dat$id == 6 & (dat$ez_trial %in% c())) 
  | (dat$id == 7 & (dat$ez_trial %in% c())) 
  | (dat$id == 8 & (dat$ez_trial %in% c())) 
  | (dat$id == 9 & (dat$ez_trial %in% c())) 
  | (dat$id == 10 & (dat$ez_trial %in% c())) 
  | (dat$id == 11 & (dat$ez_trial %in% c(1, 2, 3))) 
  | (dat$id == 12 & (dat$ez_trial %in% c())) 
  | (dat$id == 13 & (dat$ez_trial %in% c())) 
  | (dat$id == 14 & (dat$ez_trial %in% c(31))) 
  | (dat$id == 15 & (dat$ez_trial %in% c())) 
  | (dat$id == 16 & (dat$ez_trial %in% c())) 
  | (dat$id == 17 & (dat$ez_trial %in% c(12))) 
  | (dat$id == 18 & (dat$ez_trial %in% c(33))) 
  | (dat$id == 19 & (dat$ez_trial %in% c())) 
  | (dat$id == 20 & (dat$ez_trial %in% c())) 
  | (dat$id == 21 & (dat$ez_trial %in% c(54, 55))) 
  | (dat$id == 22 & (dat$ez_trial %in% c(5, 6, 26, 58))) 
  | (dat$id == 23 & (dat$ez_trial %in% c())) 
  | (dat$id == 24 & (dat$ez_trial %in% c(1:10))) 
  | (dat$id == 25 & (dat$ez_trial %in% c())) 
  | (dat$id == 26 & (dat$ez_trial %in% c()))
  | (dat$id == 27 & (dat$ez_trial %in% c())) 
  | (dat$id == 28 & (dat$ez_trial %in% c())) 
  | (dat$id == 29 & (dat$ez_trial %in% c(14))) 
  | (dat$id == 30 & (dat$ez_trial %in% c())) 
  | (dat$id == 31 & (dat$ez_trial %in% c())) 
  | (dat$id == 32 & (dat$ez_trial %in% c(8, 9)))
...
# Read Data ----
df_orig = read.csv("optotrak_data.csv")
df_orig$X <- NULL
...
# filter data
df_long %>%
  dplyr::group_by( id, block, coordinate, trial) %>%
...
df_long_clean %>% dplyr::arrange( id, condition, trial, coordinate, time) -> df_long_clean

# determine when thresholds are exceeded in primary movement axis
df_long_clean %>%
  dplyr::select( id, condition, trial, coordinate, time, centered_position) %>%
  ...
  dplyr::arrange( id, condition, trial, coordinate, time) -> df_long_moved  # the arrange function is necessary for lining up both data frames
...
velo_end = 100
df_long_clean %>%
  dplyr::select( id, condition, trial, coordinate, time, velocity) %>%
  ...
  dplyr::arrange( id, condition, trial, coordinate, time) -> df_long_velo # the arrange function is necessary for lining up both data frames
...
df_long_clean %>%
  group_by( id, condition, trial, coordinate) %>%
  dplyr::mutate(
...
df_long_trim %>%
  group_by( id, condition, coordinate, zero_time) %>%
  dplyr::summarise(
...
df_long_norm2 %>%
  dplyr::group_by(blocking,  id, condition, trial, coordinate, norm_time) %>%
  dplyr::summarize(
...
df_long_norm %>%
  dplyr::group_by(blocking,  id, condition, coordinate, trial) %>%
  dplyr::summarize(
...
# P18 appears to be the outlying one from the data
# when comparing to the other trajectories, it appears that 18 overshoots the target more often
# this would explain why the average presented below appears to peak earlier than it should
plot_norm(18, "y_inter", "norm_position", "y position (mm)")
# Z
...
# Normalized Averages ----
df_long_norm %>%
  group_by(blocking,  id, condition, coordinate, norm_time) %>%
  dplyr::summarise(
...
kms %>%
  dplyr::group_by(blocking,  id, condition, coordinate) %>%
  dplyr::summarise(
...
# P18 appears to be the outlying one from the data
plot_norm_avg(18, "y_inter", "position_avg", "y position (mm)")
# Z
...
df_long_norm %>%
  dplyr::group_by(blocking,  id, condition, coordinate, norm_time) %>%
...
df_long_norm %>%
  dplyr::group_by(blocking,  id, condition, trial) %>%
  dplyr::summarise(
    good_movement_time = unique(good_movement_time)
    , good_response_time = unique(good_response_time)
    , good_rt = unique(good_rt)
...
df_spat_var %>%
  dplyr::select( id, condition, coordinate, norm_time, spat_var) %>%
...
# plot individual
plot_discontinuities(30, "norm_velocity", "y velocity (mm/s)")
...
# plot individual
plot_discontinuities(30, "norm_acceleration", "y acceleration (mm/s^2)")
...
df_long_norm %>%
  dplyr::filter(coordinate == "y_inter") %>%
  dplyr::group_by( id, blocking, condition, trial) %>%
  dplyr::summarize(
    discontinuity_trial = unique(neg_accel_trial) | unique(neg_velo_trial)
  ) %>%
  dplyr::group_by( id, blocking, condition) %>%
  dplyr::summarise(
...
# For MATLAB ----
df_long_trim %>% 
  dplyr::select( id, blocking, condition, trial, coordinate, zero_time, centered_position) %>%
...
# write.csv(mat_long_trim, file = "./mat_long_trim.csv")
...
df_long_trimy %>%
  dplyr::group_by(blocking,  id, condition, trial, coordinate, time_lores) %>%
  dplyr::summarize(
...
# averave over trials
df_long_biny %>%
  group_by(blocking,  id, condition, coordinate, time_lores) %>%
  dplyr::summarise(
...
# # package for cluster
# save(data_for_stan, file = "./data_for_stan_15.rdata")
...
# # NOTE: the "post" stan object was generated from non-randomized participant data
# # Therefore, the object is not provided in the github repository. However, the code is left commented 
# # in the event that someone chooses to sample their own posterior distributions with the model and may
# # benefit from the code provided below. 
# 
# # load stan fit object that was computed in the cloud
# load("./post_1000_15_mtd17.rdata")
# 
# 
# 
# #### Overview
...
# load posterior samples for population estimates 
load("./post_samples_for_pop_estimates.rdata")
...
# ####   Subject Level
# 
# # NOTE: as noted earlier, the subject-level posterior samples are not provided in the 
# # github repository since the participant data is not randomized. The following commented code
# # would be used to display participant-wise posterior samples/estimates.
# 
# # Mean ----
# 
# # extract samples
...
# average over peak times 
kms %>%
  dplyr::group_by(blocking,  id, coordinate) %>%
  dplyr::summarise(
    time_peak_velocity = mean(time_peak_velocity)
    , time_peak_acceleration = mean(time_peak_acceleration)
    , time_peak_deceleration = mean(time_peak_deceleration)
